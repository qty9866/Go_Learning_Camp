			...
			http.MethodGet: {
				...
				children: map[string]*node{
					"user": {
						path: "user",
						children: map[string]*node{
							"home": {path: "home", handler: mockHandler, nodeType: nodeTypeStatic},
						},
						handler:  mockHandler,
					...
					"order": {
						path: "order",
						children: map[string]*node{
							"detail": {path: "detail", handler: mockHandler, nodeType: nodeTypeStatic},
						},
					...
					"param": {
						...
						paramChild: &node{
							...
							children: map[string]*node{"detail": {path: "detail", handler: mockHandler, nodeType: nodeTypeStatic}},
							handler:  mockHandler,
			...
			http.MethodPost: {
				...
				children: map[string]*node{
					"order": {path: "order", children: map[string]*node{
						"create": {path: "create", handler: mockHandler, nodeType: nodeTypeStatic},
					}},
					"login": {path: "login", handler: mockHandler, nodeType: nodeTypeStatic},
				},
				nodeType: nodeTypeStatic,
			},
			http.MethodDelete: {
...
func (n *node) equal(y *node) (string, bool) {
	...
	if n.nodeType != y.nodeType {
		return fmt.Sprintf("%s 节点类型不相等 x %d, y %d", n.path, n.nodeType, y.nodeType), false
	}
